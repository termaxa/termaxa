...
Termaxa works by *cooperative interception*: it sees commands that flow through the agent's hooks (Claude Code `PreToolUse`/`PostToolUse` on `Bash` and the write tools; Codex `PreToolUse`/`PostToolUse` on `Bash|apply_patch`, deny only; Cursor `beforeShellExecution`/`afterShellExecution` and, on 3.11+, `preToolUse`/`postToolUse` on `Write|Delete`; Copilot CLI's `.github/hooks`), through the shims `termaxa wrap` puts in front of a harness, or that you run via `termaxa run --`. That covers the normal path an agent takes. It does **not** cover:
...
- **An agent's own native file/edit tools, where no path rule names the file.** Since v0.19 a `Write`/`Edit`/`Delete` and a patch's file headers are judged by their target against the `match_path` rules, with insurance and a receipt when one fires; a file no rule names gets no decision, and the policy default does not apply. Observed in live testing before that: a Cursor agent, blocked on shell deletes, switched to its built-in file-delete tool and removed ~20 files that never passed through the shell hook. Every harness's write payloads are now captured and read (`docs/dialects.md`); Cursor's `Delete` passed through by default until v0.19.2.
...
If yes, it is a bug and it gets fixed regardless of threat model — a careless agent hits `rm -r -f /` and `git push -f` exactly as readily as a hostile one. If it only fires under deliberate evasion — quoting a command name to dodge a rule, a payload hidden in `python -c` or `eval`, a SQL comment inside a keyword — it is a documented limit, and this section is that documentation. (A POSIX shell's `-c` string — `sh`, `bash`, `dash`, `zsh` — is read as the command it runs, since v0.17.1 (#62), and so is `eval '…'` with a single-quoted string (v0.18.5), which is how Claude Code spells every tool call; a quoted heredoc inside `$(cat <<'EOF' … EOF)` is read as the data it is (v0.19.3). A script file, an unquoted heredoc, and the Windows shells' `-Command` strings are not read.)
...
*Implementation claims below are current as of v0.19.4. The threat model above is stable; this list is not.*

- **A script is not read.** *(architectural)* `python remove_old_mirror.py`, `bash cleanup.sh`, `node fix.js`: the gate sees the interpreter and the file name, not what the file does. Under the starter that is an ask (a human is consulted; unattended, a refusal), and that is all: no preview of what the script will delete, no insurance, and a record that says `python remove_old_mirror.py`. The Sep 20, 2026 incident that deleted about 48,000 live files through Windows directory junctions was exactly this: an agent-written Python remover, run by name. Since v0.19 the hook does see the agent *write* the script, so a later step could read a freshly written file for deletion calls and ask with that finding; that is a heuristic that fails closed and is filed, not shipped. Until then, a script an agent wrote and then runs is the biggest thing a shell gate cannot see.
- **Shell parsing is heuristic.** Termaxa splits on `&&`, `||`, `;`, `|`, a lone `&` and newlines, and detects `$(...)`/backticks, escalating those to a human unless the policy would explicitly allow what is inside (v0.19.3: `$(git branch --show-current)` is readable; `$(curl …)` is not). It does not fully parse subshells `( )`, process substitution, or variable-expanded command names, though a variable assigned in the clear earlier in the same line resolves (v0.19). Unparseable constructs are judged conservatively (the policy default, which ships as `ask`), but "conservative" is not "guaranteed."
...
- **An unrecognised payload passes through, by design; a policy can refuse it instead.** The hook's founding choice is to fail open on a payload it cannot read, because a gate that fails closed on every harness update is the outage the day an event is renamed — and that renaming has happened (Cursor 3.11, four releases ungated before v0.11.4). The cost is silence: `termaxa doctor` and the liveness probe exist to make the silence visible. Since v0.17.1 the policy can choose the other side: `unrecognised: deny` refuses any event that looks like a shell tool call (a tool named like a shell, or a `command` field) the reader could not parse, and `backup_failure: deny` refuses a command whose insurance could not be taken. Both are the right setting for an unattended run and the wrong one for a person at a terminal, which is why neither is the default.
...
- **Termaxa does not defend its own configuration.** `.termaxa/` is an ordinary directory. A command matching a read-only allow rule can overwrite `policy.yaml` with a permissive one, or truncate the audit log (which is also the circuit breaker's memory). A *malformed* policy fails closed — `Policy::load` errors and the command is blocked — so the gap is the valid-but-permissive case. As of v0.15 the file-tool path is closed for these files: `termaxa init` registers a second `PreToolUse` matcher on the write tools (`Write|Edit|MultiEdit|NotebookEdit`) which reads the target path and denies a write landing in `.termaxa/` or an agent hook config. Since v0.19 the same path goes on to judge every other target by the `match_path` rules (with `Delete` and a patch's file headers read as removals), so it is a file-write gate for the files a rule names and silence for the rest. Two things it does not reach: a hook wired by hand from a pre-v0.15 snippet, which has the shell half only; and a write tool whose name falls outside the vocabulary the matcher lists, since the matcher is the harness's filter and Termaxa cannot widen it after the fact. Cursor's registration has named `Write|Delete` since v0.19.2. `context_escalates` still runs regardless of policy, so a destructive *command* cannot reach silent `allow` even under a fully permissive policy — but it can reach `ask`.
...
- **Backups have boundaries.** `pg_dump`/`psql` and `git` must be on PATH. `rm` insurance resolves the command head, so `/bin/rm` and `/usr/bin/rm` are covered; shell aliases and variable-expanded command names are not. (This asymmetry is closed as of v0.16: the classifier, the delete extractor and the backup engine share one head resolution, so `sudo rm`, `/bin/rm`, `env rm` and the Windows path form are classified and insured alike. `xargs`-style indirection is still classified by its own rule rather than by head resolution.) Compound commands are insured for the first insurable segment only. Remote Terraform state is not backed up (its backend versions it). Since v0.19 backups are pruned by a `retention:` rule (keep 50 / 30 days by default, both conditions required, at most one per insured command and all under `termaxa backups --prune`), the copy is refused above the preview's budget rather than made uncapped, and since v0.19.4 a link is copied as a link rather than followed — a directory holding a link into a large live tree used to pass the cap as a handful of entries and then copy the whole tree behind it.
...
| **wrapped** (`termaxa wrap -- <agent>`) | every command run *through a shell resolved by name* reaches the gate, hooked or not; Claude Code is pointed at the shim through `CLAUDE_CODE_SHELL`; Codex runs its login shell by absolute path and is not reached — use its hooks | the agent's user | tamper with the record; escape via tools that execute without spawning through the wrapper — an absolute `/bin/sh`, or a direct `execve` |
...
- Use [private vulnerability reporting](https://github.com/termaxa/termaxa/security/advisories/new) for anything sensitive — it opens a private thread with the maintainers (the playground at [play.termaxa.com](https://play.termaxa.com) points here too), or
