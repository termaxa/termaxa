...
- **Claude Code and Cursor are live-tested; Codex is half-way there.** Claude Code and Cursor are exercised end-to-end, including the circuit breaker tripping under a real Cursor session. Codex's hook was captured live on Sep 5, 2026 (codex-cli 0.153.4, Windows): the hook fires, the payload is read, and Codex honours exactly one PreToolUse verdict — `deny` — so an `ask` is rendered as a deny whose reason says the gate asked and how to add an allow rule; every allow is silence. Whether that deny lands in Codex's own UI is the next measurement. The Copilot dialect parses the documented format and isn't verified end-to-end yet. Help validating it is welcome.
